        ...
        switch(opt) {
            ...
            case 'l':
                ...
                if (length > 1 && length <= 15) {
                    ...
                    printf("length of words is invalid\n");
                    return -1;
                }
                break;
            case ':':
...
char *generateMessage()
{
    ...
    for (int i = 0; i < length; i++)
    {
        ...
        add_at_index(message, word, i); // ERROR -> i+1 goes out of array bounds
    }
    ...
    for (int i = 0; i < length; i++)
    {
        ...
        size_t new_size; // ERROR -> need to check if string_message is NULL
        if (string_message != NULL) {
            new_size = strlen(string_message) + strlen(word) + 2; // ERROR -> account for space and terminator character
        } else {
            new_size = strlen(word) + 1;
        }
